...
@Service
@Slf4j
public class UserServiceImpl31 {
    @Autowired
    ...
    public ResultMsg test() {
        try {
            staffInfoMapper.insert(StaffInfo.builder()
                            .staffName("UserServiceImpl31 A")
                    .build());
//            devide();
            staffInfoMapper.insert(StaffInfo.builder()
                    .staffName(" UserServiceImpl31 B")
                    .build());
            return new ResultMsg(ResultCode.SUCCESS.getCode(), "UserServiceImpl31成功了哦",null);
        } catch (Exception e) {
            throw new EnvKyException(e.getMessage(),e,"客户端您好，UserServiceImpl31有错误");
        }
    }
